...
describe('The IDM error handling', () => {
  ...
  it('error handling is extensible by customer', () => {
    ...
    const message = FRPolicy.parsePolicyRequirement(
      property,
      test.policy,
      test.customMessage as unknown as MessageCreator,
    );
    expect(message).toBe(test.expectedString);
  });
  ...
  it('returns an object array with a human readable error and the server error', () => {
    ...
    const errorObjArr = FRPolicy.parseErrors(
      errorResponse,
      customMessage as unknown as MessageCreator,
    );
