...
test.describe('Test basic registration flow', () => {
  ...
  test(`should register user successfully and then log ou`, async ({
    page,
    browserName,
  }) => {
    ...
    // Test assertions
    expect(
      messageArray.includes('Prompt from UsernameCallback is Username')
    ).toBe(true);
    expect(
      messageArray.includes('Prompt from PasswordCallback is Password')
    ).toBe(true);
    ...
    expect(
      messageArray.includes('Prompt 4: Send me special offers and services')
    ).toBe(true);
    expect(messageArray.includes('Prompt 5: Send me news and updates')).toBe(
      true
    );
    // expect(messageArray.includes('Prompt 6: Age')).toBe(true);
    expect(messageArray.includes('Prompt 7: Select a security question')).toBe(
      true
    );
    expect(
