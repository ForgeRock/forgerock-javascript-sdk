...
test.describe('Test Basic login flow', () => {
  test(`should login successfully and then log out`, async ({
    ...
    // Test assertions
    expect(messageArray.includes('Prompt from NameCallback is User Name')).toBe(
      true
    );
    expect(
      messageArray.includes(
        'Prompt from TextInputCallback is Provide a nickname for this account'
      )
    ).toBe(true);
    expect(
    ...
    expect(
      messageArray.includes('Message for confirmation is: Is it true?')
    ).toBe(true);
