...
const defaultMessageCreator: MessageCreator = {
  ...
  [PolicyKey.CannotContainOthers]: (
    property: string,
    params?: { disallowedFields?: string }
  ) => {
  ...
  [PolicyKey.LeastCapitalLetters]: (
    property: string,
    params?: { numCaps?: number }
  ) => {
    const numCaps = getProp<number>(params, 'numCaps', 0);
    return `${property} must contain at least ${numCaps} capital ${plural(
      numCaps,
      'letter'
    )}`;
  },
  [PolicyKey.LeastNumbers]: (
    property: string,
    params?: { numNums?: number }
  ) => {
    const numNums = getProp<number>(params, 'numNums', 0);
    return `${property} must contain at least ${numNums} numeric ${plural(
      numNums,
      'value'
    )}`;
  },
  ...
  [PolicyKey.MaximumLength]: (
    ...
    return `${property} must be at most ${maxLength} ${plural(
      maxLength,
      'character'
    )}`;
  },
  ...
  [PolicyKey.MinimumLength]: (
    ...
    return `${property} must be at least ${minLength} ${plural(
      minLength,
      'character'
    )}`;
  },
  ...
  [PolicyKey.UnknownPolicy]: (
    ...
    const policyRequirement = getProp<string>(
      params,
      'policyRequirement',
      'Unknown'
    );
