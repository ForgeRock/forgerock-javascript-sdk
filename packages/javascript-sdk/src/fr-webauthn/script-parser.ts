...
function parseWebAuthnRegisterText(
  text: string
  ...
  const attestation = getIndexOne(
    text.match(/attestation"{0,}:\s{0,}"(\w+)"/)
  ) as AttestationType;
  // e.g. `timeout: 60000`
  ...
  const userId = getIndexOne(
    user.match(/id"{0,}:\s{0,}Uint8Array.from\("([^"]+)"/)
  );
  ...
  const userDisplayName = getIndexOne(
    user.match(/displayName"{0,}:\s{0,}"([\d\w\s.@_-]+)"/)
  );
...
function parseWebAuthnAuthenticateText(
  text: string
  ...
  if (allowCredentialsText) {
    ...
    const allowCredentialArr = allowCredentialsText.split('},') || [
      allowCredentialsText,
    ];
    // Iterating over array of substrings
    allowCredentials = allowCredentialArr.map((str) => {
      // e.g. `{ \"type\": \"public-key\",
      const type = getIndexOne(
        str.match(/type"{0,}:\s{0,}"([\w-]+)"/)
      ) as 'public-key';
      // e.g. \"id\": new Int8Array([-107, 93, 68, -67, ... -19, 7, 4]).buffer
      const idArr = ensureArray(
        str.match(/id"{0,}:\s{0,}new\s{0,}(Uint|Int)8Array\(([^\)]+)/)
      );
